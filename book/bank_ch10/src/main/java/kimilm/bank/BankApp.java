...
public class BankApp {
	...
	public static void main(String args[]) throws Exception {
		...
		// -- create another fixed deposit. The createFixedDeposit method is annotated with
		// @CacheEvict; therefore, all cached FixedDeposits are evicted from cache
		fixedDepositService.createFixedDeposit(fdd);
		...
		for (FixedDepositDetails detail : fixedDepositDetailsList) {
			// -- getFixedDeposit method is annotated with @CachePut; therefore, is always invoked
			fixedDepositService.getFixedDeposit(detail.getFixedDepositId());
		}

		for (FixedDepositDetails detail : fixedDepositDetailsList) {
			// -- the getFixedDepositFromCache is never invoked because the earlier for loop cached all the fixed deposits
